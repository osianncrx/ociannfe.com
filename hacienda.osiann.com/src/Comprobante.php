...
class Comprobante
{
    ...
    public $consecutivo = ''; //Consecutivo del comprobante
    ...
    public function __construct($container, $datos, $id, $sinInternet = false)
    {
        ...
        if (isset($datos['clave']) && isset($datos['tipo'])) {
            ...
            $stmt = $db->prepare(
                "SELECT estado, $id_name, NumeroConsecutivo FROM $tabla
                WHERE clave=?"
            );
            if ($stmt === false) {
    ...
    public function guardarEnCola()
    {
        ...
        if ($hasRecord) {
            ...
            //Guardar el registro
            $sql = "INSERT INTO $table (clave, NumeroConsecutivo, id_empresa, estado)
            VALUES ('$clave', '{$this->consecutivo}', '{$this->id}', 1)";
            $db->query($sql);
